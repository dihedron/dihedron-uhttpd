...
public class RequestDispatcher {
	...
	public class Dispatch {
		...
		public RequestDispatcher through(Handler handler) throws ApplicationException {
			...
			dispatcher.handlers.add(new HandlerInfo(regex, handler));
			return dispatcher;
		}
	}
	...
	public class Forward {
		...
		public RequestDispatcher goTo(String url) throws ApplicationException {
			...
			dispatcher.errors.add(new ErrorInfo(new Error(clazz), url));
			return dispatcher;
		}
	}	
	
	private class HandlerInfo {
		private HandlerInfo(Regex regex, Handler handler) {
			this.regex = regex;
	...
	private class ErrorInfo {
		private ErrorInfo(Error error, String url) {
			this.error = error;
			this.url = url;
		}
	...
	public RequestDispatcher(SessionGenerator generator) {
		handlers = Collections.synchronizedList(new ArrayList<HandlerInfo>());
		errors = Collections.synchronizedList(new ArrayList<ErrorInfo>());
	...
	public void dispatch(Request request, Response response) throws ServerException {
		...
		try {			
			...
			for(ErrorInfo info : errors) {
				if(info.getError().isSuperClassOf(e)) {
					forward = info.getUrl();
				}
			}			
...
//		}
		for(HandlerInfo info : handlers) {
			if(info.getRegex().matches(target)) {
				logger.info("dispatching request to handler for '{}'", info.getRegex());
